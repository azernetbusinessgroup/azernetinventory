...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (languageSaveBtn) {
    languageSaveBtn.addEventListener('click', async () => {
      const selectedLang = languageSelector ? languageSelector.value : 'en';
      console.log('Save button clicked, selected language:', selectedLang);
...
function showUnifiedSaleConfirmation(salesData) {
  ...
  const modalTitle = document.getElementById('saleConfirmationTitle');
  const totalPriceLabel = document.getElementById('totalPriceLabel');
  
  if (modalTitle) {
    modalTitle.textContent = getTranslation('modals.saleConfirmation.title') || 'Sale Successful!';
  }
  
  if (totalPriceLabel) {
    const totalText = getTranslation('modals.saleConfirmation.totalPrice') || 'Total Price:';
    const currency = getTranslation('modals.saleConfirmation.currency') || 'ETB';
    totalPriceLabel.innerHTML = `${totalText} <span id="confirmationTotal">${Math.round(totalPrice)}</span> ${currency}`;
  }
...
function prefillSettingsForm(settingsType) {
  if (settingsType === 'accounts') {
    ...
    if (currentLanguageDisplay) {
      currentLanguageDisplay.textContent = getTranslation(`settings.language.languages.${currentLanguage}`) || 'English';
    }
  }
}
